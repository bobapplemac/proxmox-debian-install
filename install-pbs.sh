...
# Quick start:
# curl -fsSLO https://raw.githubusercontent.com/bobapplemac/proxmox-debian-install/main/install-pbs.sh && bash install-pbs.sh
...
#
#              Containers are not supported. Virtual machines receive the minimal
#              proxmox-backup-server package and retain the Debian kernel. Bare-metal systems
#              receive the proxmox-backup meta package, including the Proxmox kernel and the
#              fuller package set normally used by the Proxmox Backup Server installer.
#
#              The pbs-no-subscription repository is configured using Debian deb822 syntax and
#              the Proxmox Trixie archive key is verified by SHA256 before installation. Any PBS
...
#              Proxmox Backup Server packages
#
# Notes:
#              This script is intended for fresh Proxmox Backup Server 4 installations on
#              Debian 13 (Trixie). It intentionally does not convert Netplan, assign a static IP,
#              alter /etc/hosts, install VM guest tools, or configure backup datastores.
#
#              Current Proxmox Backup Server packages recommend ifupdown2. A system beginning with
...
#
#              On bare-metal PBS installations, the full Proxmox package set installs the Proxmox
...
#
#              Normal VM mode installs proxmox-backup-server and retains the Debian kernel. It does
#              not perform GRUB preparation, GRUB installation, kernel cleanup, update-grub, or
#              /boot/EFI verification. --force-bare-metal selects the exact same bare-metal path
...
PBS_REPO=/etc/apt/sources.list.d/proxmox.sources
...
network_stack_error() {
    echo "ERROR: $*" >&2
    echo >&2
    echo "Proxmox Backup Server installation requires persistent networking to be" >&2
    ...
    echo "it with ifupdown2 as part of the Proxmox Backup Server installation." >&2
    exit 1
}
...
static_network_error() {
    echo "ERROR: $*" >&2
    echo >&2
    echo "Proxmox Backup Server requires a correctly configured static IPv4 address." >&2
    echo >&2
    ...
    echo "before installing Proxmox Backup Server." >&2
    exit 1
}
...
check_requirements() {
    ...
    for cmd in \
        apt-get awk debconf-set-selections dpkg dpkg-query find getent grep hostname install \
        ip mktemp rm sed sha256sum sort systemctl systemd-detect-virt uname wget
    do
...
check_fresh_install() {
    if package_installed proxmox-backup-server || package_installed proxmox-backup; then
        die "Proxmox Backup Server packages are already installed. This script is intended for a fresh Debian installation."
    fi
}
...
check_network_stack() {
    ...
    if systemctl is-active --quiet systemd-networkd.service 2>/dev/null; then
        network_stack_error "systemd-networkd.service is still active. Reboot after completing the ifupdown conversion before installing PBS."
    fi
...
check_virtualization() {
    ...
    if detected=$(systemd-detect-virt --container 2>/dev/null); then
        die "Container virtualization ('$detected') was detected. Proxmox Backup Server must not be installed in a container."
    fi
...
configure_proxmox_repository() {
    echo
    echo "Configuring Proxmox Backup Server no-subscription repository..."
...
simulate_pbs_install() {
    local simulation

    echo
    echo "Simulating Proxmox Backup Server package installation..."
    ...
    simulation=$(
        ...
        printf '%s\n' "$simulation" >&2
        die "APT could not simulate the Proxmox Backup Server installation."
    }
    ...
    if grep -Eq '^Remv ifupdown2([[:space:]]|$)' <<< "$simulation"; then
        ...
        printf '%s\n' "$simulation"
        echo
        die "APT would remove ifupdown2 during the Proxmox Backup Server installation."
    fi
    ...
    if grep -Eq '^Inst ifupdown([[:space:]]|$)' <<< "$simulation"; then
        ...
        printf '%s\n' "$simulation"
        echo
        die "APT would reinstall classic ifupdown during the Proxmox Backup Server installation."
    fi
...
install_pbs_packages() {
    ...
    package_installed proxmox-backup-server ||
        die "proxmox-backup-server does not appear to be installed successfully."

    package_installed ifupdown2 ||
        die "ifupdown2 is not installed after Proxmox Backup Server installation."

    ! package_installed ifupdown ||
        die "Classic ifupdown is unexpectedly installed after Proxmox Backup Server installation."

    if [[ $SYSTEM_TYPE == "bare metal" ]]; then
        package_installed proxmox-backup ||
            die "proxmox-backup does not appear to be installed successfully."

        package_installed proxmox-default-kernel ||
            die "proxmox-default-kernel was not installed by the bare-metal PBS package set."
    fi

    command -v proxmox-backup-manager >/dev/null 2>&1 ||
        die "proxmox-backup-manager was not found after installation."
}
...
disable_enterprise_repository() {
    echo
    echo "Disabling Proxmox Backup Server enterprise repository when present..."

    disable_deb822_enterprise_source "$PBS_ENTERPRISE_SOURCES" "PBS enterprise repository"
    disable_legacy_enterprise_source "$PBS_ENTERPRISE_LIST" "legacy PBS enterprise repository"
...
verify_installation() {
    ...
    package_installed proxmox-backup-server ||
        die "Verification failed: proxmox-backup-server is not installed."
    ...
    systemctl is-active --quiet proxmox-backup.service ||
        die "Verification failed: proxmox-backup.service is not active."

    systemctl is-active --quiet proxmox-backup-proxy.service ||
        die "Verification failed: proxmox-backup-proxy.service is not active."

    systemctl is-enabled --quiet fstrim.timer ||
        die "Verification failed: fstrim.timer is not enabled."

    daemon_state=$(systemctl is-active proxmox-backup.service)
    proxy_state=$(systemctl is-active proxmox-backup-proxy.service)

    echo
    echo "Proxmox Backup Server installation verification:"
    echo
    ...
    printf '  %-20s %s\n' "PBS daemon:" "$daemon_state"
    printf '  %-20s %s\n' "PBS proxy:" "$proxy_state"
    printf '  %-20s %s\n' "fstrim:" "$(systemctl is-enabled fstrim.timer)"
    printf '  %-20s %s\n' "Web interface:" "https://$HOST_FQDN:8007"
    printf '  %-20s %s\n' "Web interface (IP):" "https://$LOCAL_IPV4:8007"

    echo
    echo "Installed PBS versions:"
    echo
    proxmox-backup-manager versions | sed 's/^/  /'
}

finish_installation() {
    echo
    echo "Proxmox Backup Server installation is complete."
    echo
    ...
    if [[ $SYSTEM_TYPE == "bare metal" ]]; then
        ...
        echo "A reboot is recommended after the Debian upgrade and PBS installation."
    fi
    ...
    if yesno "Reboot now?" Y; then
        ...
        echo
        if [[ $SYSTEM_TYPE == "bare metal" ]]; then
            echo "Reboot the system before putting Proxmox Backup Server into service."
        else
            echo "Reboot the system when convenient before putting Proxmox Backup Server into service."
        fi
    fi
}
...
main() {
    ...
    echo
    yesno "Install Proxmox Backup Server using the configuration shown above?" N || exit 0
